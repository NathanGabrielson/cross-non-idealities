...
def crossbar_time(Ri, layout_array, Runselhi, Runsellow, plot_code, 
  ...
  for time in range(t_start, t_end):
    if (time % 2 != 0): #If Odd
      layout_array[:,::2] = very_high #: selects every row, while ::2 selects every element before the 2 in each row. Very high is equal to 10e15 or something
      layout_array[:,1::2] = copy[:,1::2] #Adjusts the layout array I guess?
    else:
...
def crossbar(Ri, layout_array, Runselhi, Runsellow, plot_code, 
    ...
    if(plot_code == 'current_obs'): 
      array_size = len(layout_array[0]) #Creates a vector of length n
      
      obs_array = np.zeros((array_size,array_size)) #Creates an empty matrix of size NxN
      ...
      for i in range(0,array_size):
        ...
        wlbiasl_temp[i] = wlbiasl[i] #fills the temp wordline with the biases given 
...
def my_function(Ri, layout_array, Runselhi, Runsellow, 
    ...
    Rbiasl[Rbiasl == 0] = Runselhi #Sets every value equal to 0 to a very high resistance
    Rbiasl[Rbiasl != Runselhi] = Runsellow #Sets every other value to a low resistance
    ...
    matrix_a = np.zeros((array_size**2, array_size**2), #Creates an empty Array of Size N^2xN^2
                        dtype='float32')
